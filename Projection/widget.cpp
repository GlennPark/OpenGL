    ...
    : QOpenGLWidget(parent)
{
    ...
    left = -1, right = 1, bottom = -1, top = 1, bNear = -1, bFar = 1; fov = 45;

    setWindowTitle("OpejGL Viewing");
    resize(600, 600);
}
...
void Widget::keyPressEvent(QKeyEvent* event)
{
    ...
    QString str = QString("%.0f, %.0f, %.0f, %.1f, %.1f, %.1f, %.1f, %.1f, %.1f")
            .arg(xAngle).arg(yAngle).arg(zAngle).arg(left).arg(right).arg(bottom).arg(top)
            .arg(bNear).arg(bFar);
